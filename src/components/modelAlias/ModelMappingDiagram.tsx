...
const areDiagramLinesEqual = (left: DiagramLine[], right: DiagramLine[]): boolean => {
  if (left.length !== right.length) return false;
  return left.every((line, index) => {
    const nextLine = right[index];
    return line.id === nextLine.id && line.path === nextLine.path && line.color === nextLine.color;
  });
};
...
export const ModelMappingDiagram = forwardRef<ModelMappingDiagramRef, ModelMappingDiagramProps>(
  function ModelMappingDiagram(
    ...
    const { aliasNodes, providerNodes } = useMemo(() => {
      ...
      const aliasSourcesMap = new Map<string, SourceNode[]>();
      sources.forEach((source) => {
        source.aliases.forEach((entry) => {
          const aliasSources = aliasSourcesMap.get(entry.alias);
          if (aliasSources) {
            aliasSources.push(source);
          } else {
            aliasSourcesMap.set(entry.alias, [source]);
          }
        });
      });
      ...
      const aliasNodesList: AliasNode[] = Array.from(aliasSet)
        .map((alias) => ({
          id: alias,
          alias,
          sources: aliasSourcesMap.get(alias) ?? [],
        }))
        .sort((a, b) => {
    ...
    const updateLines = useCallback(() => {
      ...
      const aliasRectCache = new Map<string, DOMRect>();
      ...
      const getAliasRect = (alias: string) => {
        const cached = aliasRectCache.get(alias);
        if (cached) return cached;
        const aliasEl = aliasRefs.current.get(alias);
        if (!aliasEl) return null;
        const rect = aliasEl.getBoundingClientRect();
        aliasRectCache.set(alias, rect);
        return rect;
      };
      ...
      providerNodes.forEach(({ provider, sources }) => {
        ...
        const sourceRectCache = new Map<string, DOMRect>();
        ...
        sources.forEach((source) => {
          ...
          sourceRectCache.set(source.id, sourceRect);
        ...
        sources.forEach((source) => {
          ...
          const sourceRect = sourceRectCache.get(source.id);
          if (!sourceRect) return;
          const x1 = sourceRect.right - containerRect.left;
          const y1 = sourceRect.top + sourceRect.height / 2 - containerRect.top;

          source.aliases.forEach((aliasEntry) => {
            const aliasRect = getAliasRect(aliasEntry.alias);
            if (!aliasRect) return;
      ...
      setLines((prev) => (areDiagramLinesEqual(prev, newLines) ? prev : newLines));
