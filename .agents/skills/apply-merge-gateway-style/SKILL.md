...
- Never increase border width on inputs, search fields, buttons, selects, filter controls, or any other interactive control in hover, focus-visible, active/pressed, selected, or open states. Keep input/search border thickness and color constant across these states; other controls may communicate state through border color, neutral fill, or an external outline/focus ring that does not alter layout.
